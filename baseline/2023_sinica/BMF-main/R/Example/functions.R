...
zero <- function(x) {
  mean_val <- mean(x)
  return(mean_val * (abs(mean_val) > 1e-5))
}
...
parameter_sigma <- function(k1, k2, n, u, Q, df, alpha, beta, data, data_mod, w, w_mod, A21_mat, A22_mat, tao11, tao21, tao22, A_upper, A_lower) {
  # Debug: Check input dimensions
  cat("Inside parameter_sigma, dim of data:", dim(data), "\n")
  cat("Inside parameter_sigma, dim of data_mod:", dim(data_mod), "\n")
  cat("Inside parameter_sigma, dim of w:", dim(w), "\n")
  cat("Inside parameter_sigma, dim of w_mod:", dim(w_mod), "\n")
  
  # Ensure data has correct dimensions
  if (nrow(data) != (3*k1 + k2)) stop("parameter_sigma: data must have ", 3*k1 + k2, " rows")
  if (nrow(w) != (3*k1 + k2)) stop("parameter_sigma: w must have ", 3*k1 + k2, " rows")
  
  # Compute high-frequency matrices using data (237 rows)
  s1 <- data[,1:n] %*% t(data[,1:n])  # 237 x 237
  s2 <- data[,2:(n + 1)] %*% t(data[,1:n])  # 237 x 237
  s3 <- data[,2:(n + 1)] %*% t(data[,2:(n + 1)])  # 237 x 237
  mat <- s3 - (s2 %*% t(w)) - t(s2 %*% t(w)) + (w %*% s1 %*% t(w))  # 237 x 237
  cat("Inside parameter_sigma, dim of mat:", dim(mat), "\n")
  
  # Extract high-frequency block
  mat_1st_block <- mat[1:(3*k1), 1:(3*k1)]  # 228 x 228
  cat("Inside parameter_sigma, dim of mat_1st_block:", dim(mat_1st_block), "\n")
  ...
  return(list(omega, sigma2, sigma_adj))
}
