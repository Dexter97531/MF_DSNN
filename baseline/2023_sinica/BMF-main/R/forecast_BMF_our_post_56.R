...
#==========Input Parameters=============
k1 <- 56    # Number of monthly variables
...
train_size <- 188 # Number of training observations
test_size <- 20 # Number of testing/forecast observations
...
data_full <- as.matrix(read.csv("D:/Code/MF/data/data3_197301_202503_56_9_std_reorder_v2.csv", header = FALSE))
...
# Compute errors
mae_metrics <- mean_absolute_error_lf(forecast_true_lf[1:Q1,], mean_forecast_lf[1:Q1,])
rmse_metrics <- root_mean_squared_error_lf(forecast_true_lf[1:Q1,], mean_forecast_lf[1:Q1,])

# Print results
cat("\n=== Low-Frequency Variables Forecast Results ===\n")
cat('forecast_true_lf[1:Q,]',forecast_true_lf[1:Q1,])
cat('mean_forecast_lf[1:Q,]',mean_forecast_lf[1:Q1,])
...
write.csv(forecast_true_lf[1:Q1, , drop = FALSE],
          file = "./result/true_post_BMF_lag1.csv",
          row.names = TRUE)   # keeps row names / time indices if they exist

write.csv(mean_forecast_lf[1:Q1, , drop = FALSE],
          file = "./result/forecast_post_BMF_lag1.csv",
          row.names = TRUE)

# Read back
true_BMF_lag1 <- read.csv("./result/true_post_BMF_lag1.csv", row.names = 1, check.names = FALSE)
fc_BMF_lag1   <- read.csv("./result/forecast_post_BMF_lag1.csv", row.names = 1, check.names = FALSE)

# Convert back to matrix if needed
true_BMF_lag1 <- as.matrix(true_BMF_lag1)
fc_BMF_lag1   <- as.matrix(fc_BMF_lag1)
