library(LPTime)
data <- read.csv("D:/Code/MF/BMF-main/R/data1.csv") # Read real data (Data 1), would be similar for Data 2/Data 3
data <- data[,-1]
k1 <- 24
k2<- 9
data_hh <- data[1:(3*k1),]
...
mat <- matrix(0,nrow=k1,ncol=239)
for(i in 1:239){
  m <- NULL
  for(j in 1:k1){
    temp <- sum(data_hh[,i][((3*j)-2)],data_hh[,i][((3*j)-1)],data_hh[,i][(3*j)])/3    #三个月取平均！
    m <- c(m,temp)
  }
  mat[,i] <- m
...
index=0
horizon <- 10
num <- 1 # no. of estimation samples
...
for(k in 1:num){
  ...
  data <- t(data)
  for(i in 1:(k1+k2)){
    data[,i] <- ts(data[,i],start = c(1960,1),frequency = 4)
  }
  data
  est <- VAR(data,p=1)
