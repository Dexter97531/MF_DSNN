...
train_size <- 168 # Number of training observations
...
write.csv(forecast_true_lf[1:Q1, , drop = FALSE],
          file = "./result/true_BMF_lag1.csv",
          row.names = TRUE)   # keeps row names / time indices if they exist

write.csv(mean_forecast_lf[1:Q1, , drop = FALSE],
          file = "./result/forecast_BMF_lag1.csv",
          row.names = TRUE)

# Read back
true_BMF_lag1 <- read.csv("./result/true_BMF_lag1.csv", row.names = 1, check.names = FALSE)
fc_BMF_lag1   <- read.csv("./result/forecast_BMF_lag1.csv", row.names = 1, check.names = FALSE)
