...
class EnvConfigurator():
    ...
    def config_args(self, pickle_args = True):
        ...
        # xdata, ydata = x.values, y.values
        data_full = pd.read_csv(f"D:\Code\MF\data\data3_196001_202503_76_9.csv", index_col=0)

        # Generate timestamps
        start_date = pd.to_datetime("1960-01-01")
        n_time_points = data_full.shape[1]
        monthly_timestamps = pd.date_range(start=start_date, periods=n_time_points*3, freq='M')
        ...
        P = 76  # Number of high-frequency variables (from R code)
