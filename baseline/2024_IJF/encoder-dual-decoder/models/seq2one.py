...
from tensorflow.keras.layers import Layer, Concatenate, Dot, Input, LSTM, Dropout
...
class ConvertTensorLayer(Layer):
    ...
    def call(self, inputs):
        ...
        batch_size = tf.shape(x)[0]  # Extract batch size within the layer
        return x, y, batch_size

class InitializeStateLayer(Layer):
    def __init__(self, dim, **kwargs):
        super(InitializeStateLayer, self).__init__(**kwargs)
        self.dim = dim
...
class ExpandDimsLayer(Layer):
    def __init__(self, time_step, axis=1, **kwargs):
        super(ExpandDimsLayer, self).__init__(**kwargs)
        self.time_step = time_step
        self.axis = axis

    def call(self, inputs):
        return tf.expand_dims(inputs[:, self.time_step, :], axis=self.axis)
...
class MTMFSeq2One(tf.keras.Model):
    def __init__(
        ...
        self.dropout_fn = Dropout(dropout_rate)
        self.ffn2 = Dense(dim_y, name='y_output', kernel_regularizer=regularizers.L1L2(l1=self.l1reg, l2=self.l2reg))
        self.initialize_s = InitializeStateLayer(n_s)
        self.initialize_c = InitializeStateLayer(n_s)
        self.expand_dims_layers = [ExpandDimsLayer(t) for t in range(Ty)]
        print(f"expand_dims_layers: {[layer.time_step for layer in self.expand_dims_layers]}")  # Debug
    ...
    def call(self, batch_inputs, training=False):
        """Forward pass"""
        x, y = batch_inputs  # Encoder input, decoder input
        x, y, batch_size = self.convert_layer([x, y])  # Convert inputs using custom layer
        # batch_size = tf.shape(x)[0]
        ##############################################
        # Stage 1: Pre-attn encoding
        ##############################################
        ...
        # s = self.initialize_state(batch_size, self.n_s)
        # c = self.initialize_state(batch_size, self.n_s)
        s = self.initialize_s(batch_size)
        c = self.initialize_c(batch_size)
        for t in range(self.Ty):
            if t >= y.shape[1]:
                raise ValueError(f"Index t={t} exceeds y time dimension {y.shape[1]}")
            a_idx = int((t + 1) * self.freq_ratio - 1)
            a_to_attend = a[:, (a_idx - self.freq_ratio + 1):(a_idx + 1), :]
            context = self.one_step_attention([a_to_attend, s])
            # Context + teacher forcing, post-attention LSTM input
            expanded_y = self.expand_dims_layers[t](y)
            post_attn_input = Concatenate(axis=-1)([context, expanded_y])
            # Post-attention LSTM cell to the "context" vector
            s, _, c = self.post_attn(post_attn_input, initial_state=[s, c])
        
        y_pred = self.ffn1(s)
        y_pred = self.dropout_fn(y_pred, training=training)
        y_pred = self.ffn2(y_pred)
        print("call output shapes in seq2one:", x_pred.shape, y_pred.shape)  # Debug print
        return [x_pred, y_pred]
    ...
    #     return Model(inputs=[x_input, y_input], outputs=self.call([x_input, y_input]))
    def build_graph(self):
            x_input = Input(shape=(self.Lx, self.dim_x), name='x_input')
            y_input = Input(shape=(self.Ty, self.dim_y), name='y_input')
            x_pred, y_pred = self.call([x_input, y_input])
            return Model(
                inputs=[x_input, y_input],
                outputs={'x_output': x_pred, 'y_output': y_pred}
            )
