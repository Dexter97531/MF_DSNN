...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
            """
            Build method to initialize the layer's weights.
            Argv:
                input_shape: Shape of the input tensor (batch_size, Lx, dim_x)
            """
            # self.mha.build([input_shape, input_shape, input_shape])
            self.mha.build(query_shape=input_shape, value_shape=input_shape)
            self.layernorm1.build(input_shape)
            # ffn1_input_shape = input_shape[:-1] + (self.mha.key_dim * self.mha.num_heads,)
            # self.ffn1.build(ffn1_input_shape)
            # self.dropout_fn.build(ffn1_input_shape)
            # self.ffn2.build(ffn1_input_shape)
            self.ffn1.build(input_shape)
            self.dropout_fn.build(input_shape)
            self.ffn2.build(input_shape)
    ...
    def call(self, x, training, look_ahead_mask = None):
        ...
        attn_output = self.mha(query=x, value=x, key=x, attention_mask=look_ahead_mask)  # self-attention (batch_size, Lx, dim_x)
...
class Encoder(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        Argv:
            input_shape: Shape of the input tensor (batch_size, Lx, dim_x)
        """
        self.pos_encoding = tf.ensure_shape(self.pos_encoding, [self.Lx, self.dim_x])
    ...
    def call(self, x, look_ahead_mask=None, training = False):
        ...
        for i in range(self.num_layers):
            x = self.enc_layers[i](x, training=training, look_ahead_mask=look_ahead_mask)
...
class xDecoder(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        Build method to initialize the layer's weights.
        Argv:
            input_shape: Shape of the input tensor (batch_size, Tx, dim_x)
        """
        self.pos_encoding = tf.ensure_shape(self.pos_encoding, [self.Tx, self.dim_x])
        for layer in self.dec_layers:
            layer.build(input_shape)
        self.ffn.build(input_shape)
        super(xDecoder, self).build(input_shape)
    ...
    def call(self, x, enc_output, look_ahead_mask, training = False):
        ...
        attn_weights = {}
        for i in range(self.num_layers):
            x, attn_w1, attn_w2 = self.dec_layers[i](x, enc_output, training=training, look_ahead_mask=look_ahead_mask)
...
class yDecoder(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        Build method to initialize the layer's weights.
        Argv:
            input_shape: Shape of the input tensor (batch_size, Ty, dim_y)
        """
        self.pos_encoding = tf.ensure_shape(self.pos_encoding, [self.Ty, self.dim_y])
        for layer in self.dec_layers:
            layer.build(input_shape)
        self.ffn.build(input_shape[-2:])  # Shape for last time step: (batch_size, dim_y)
        super(yDecoder, self).build(input_shape) 
    def call(self, y, enc_output, look_ahead_mask, training = False):
        ...
        for i in range(self.num_layers):
            ...
            y, attn_w1, attn_w2 = self.dec_layers[i](y, enc_output, training=training, look_ahead_mask=look_ahead_mask)
            attn_weights[f'decoder_layer_{i}_self_att'] = attn_w1
            attn_weights[f'decoder_layer_{i}_decenc_att'] = attn_w2
...
class ConvertTensorLayer(tf.keras.layers.Layer):
    def __init__(self, **kwargs):
        super(ConvertTensorLayer, self).__init__(**kwargs)

    def call(self, inputs):
        x_encoder_in, x_decoder_in, y_decoder_in = inputs
        x_encoder_in = tf.convert_to_tensor(x_encoder_in)
        x_decoder_in = tf.convert_to_tensor(x_decoder_in)
        y_decoder_in = tf.convert_to_tensor(y_decoder_in)
        batch_size = tf.shape(x_encoder_in)[0]  # Extract batch size within the layer
        return x_encoder_in, x_decoder_in, y_decoder_in, batch_size
...
class Transformer(tf.keras.Model):
    def __init__(
        ...
        # Custom layer for tensor conversion
        self.convert_layer = ConvertTensorLayer()
    ...
    def call(self, batch_inputs, training=False):
        ...
        x_encoder_in, x_decoder_in, y_decoder_in, batch_size = self.convert_layer(batch_inputs)
        ...
        x_pred, attn_weights_xdec = self.xdecoder(x_decoder_in, enc_output, look_ahead_mask_xdecoder, training=training)
        
        ## stage 2.2: transformer decoder for y
        look_ahead_mask_ydecoder = self.create_look_ahead_mask(y_decoder_in.shape[1])
        y_pred, attn_weights_ydec = self.ydecoder(y_decoder_in, enc_output, look_ahead_mask_ydecoder, training=training)
